    ...
    static function input($type,$name,$attribute = [])
    {
        ...
        if(substr($type,0,7) == 'options')
        {
            ...
            if(substr($type, 8,3) == 'obj')
            {
                ...
                {
                    $label = '';
                    if(strpos($obj_array[2],'function') > -1)
                    {
                        eval('$fun = '.$obj_array[2]);
                        $label = $fun($option);
                    }
                    else
                    {
                        $label = $option->{$obj_array[2]};
                    }
                    $lists .= "<option value='".$option->{$obj_array[1]}."' ".($option->{$obj_array[1]}==$value?'selected=""':'').">".$label."</option>";
                }
            }
            else
            {
